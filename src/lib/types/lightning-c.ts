...
interface LnNodeNetworkAddress {
  ...
  port?: number | string; //9735;
}
/**
 * Optional info is usaually provided for our node
 */
...
export interface CreatedInvoice {
  ...
  // lnd and newer invoices require this for MMP
  payment_address?: string; // base64
}
...
export interface DecodedBolt11 {
  ...
  payment_hash: string; //"c6ca9e52649c58079caa0d47d27a810332bb0b4ea9e3c2fd8e9fa16c99407edf";
  // FIXME is this needed , not provided by lnd
...
export interface LnConnectAndFundPayload {
  peer: string;
  // NOTE: If opening and funding a channel between lnd <-> clightning minimum amount should be 57202 SATS
  // or else will rsult in a dust error
  // see:  https://github.com/ElementsProject/lightning/pull/2475
  msatoshi: number; // msatoshis "100000";
  callbackUrl?: string;
  min_confs?: number;
  // either sat_per_byte or target_conf
  sat_per_byte?: number;
  target_conf?: number;
}
...
export interface LnListPaysPayload {
  bolt11: Bolt11String;
  status: "failed" | "complete" | "inflight" | "unknown";
  amount_sent_msat: string; // "0msat";
  preimage?: string;
}
export interface LnInvoiceFilters {
  paymentLabel?: string;
  paymentHash?: string;
  pendingOnly?: boolean;
  indexOffset?: number;
...
export interface SifirLnClientInterface {
  ...
  getInvoice(invoiceFilters?: LnInvoiceFilters): Promise<CreatedInvoice[]>;
  ...
  withdrawFunds(
    destination: string,
    satoshi: number,
    //It can be one of the strings urgent (aim for next block), normal (next 4 blocks or so) or slow (next 100 blocks or so)
