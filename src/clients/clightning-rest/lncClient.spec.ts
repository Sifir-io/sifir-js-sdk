...
test("Should be able to another nodes info", async t => {
  ...
  const nodeInfo = await getNodeInfo(
    "0308dbd05278e5802dd36436a41b226824283526eb14a08d334cbbc878243b243c"
  );
  t.true(!!nodeInfo.id.length);
...
test("Should be able to create an invoice", async t => {
  ...
  t.true(!!body);
  const invoice = body;
  t.is(invoice.id, lightingInvoiceLabel);
...
test("Should be able to decode a bolt", async t => {
  ...
  const bolt11 =
    "";
...
// FIXME 404 ?
test.skip("Should be able to delete a created invoice", async t => {
  const {
    context: { deleteInvoice, lightingInvoiceLabel }
  } = t;
  const invoice = await deleteInvoice(lightingInvoiceLabel);
  t.true(invoice.label === lightingInvoiceLabel);
});
...
test("Should be able no route yet between node and an arbitrary node", async t => {
  ...
  const routes = await getRoute(
    "02eadbd9e7557375161df8b646776a547c5cbc2e95b3071ec81553f8ec2cea3b8c", // Peer of LNC
    10,
    1
  );
...
test("Should be able listFunds (outputs and channels)", async t => {
  ...
  t.true(!!funds.channels.every(channel => channel.state));
